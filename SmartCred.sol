...
contract SmartCredit {
    ...
    */
    function addTransaction(address wallet_address, Transaction calldata transaction) public {
        ...
        if (transaction.status == TransactionStatus.LOAN) {
            borrowers[wallet_address].outstandingDebt += transaction.amount;
        } else if (transaction.status == TransactionStatus.DEFAULTED) {
            borrowers[wallet_address].numMonthsSinceLastDefault = 0;
        } else if (transaction.status == TransactionStatus.PAID) {
            borrowers[wallet_address].outstandingDebt -= transaction.amount;
        } else {
            borrowers[wallet_address].numRecentInquiries += 1;
        }
    }
    ...
    function getTransactionHistory(address wallet) public view
